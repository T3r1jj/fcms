...
public class ReplicationScheduler {
    ...
    void sendSampleNotification() {
        historyService.addAndNotify(new Event.Builder()
                ...
                .setPayload(new Payload(new StoredRecord(ObjectId.get(), new StoredRecordMeta(ObjectId.get(),"b","c", "d")), Payload.Type.values()[new Random().nextInt(Payload.Type.values().length)]))
                .build());
    }
}
